...
export async function migrate() {
    ...
    if (!isIntitialized) {
        ...
        let stepVersionResult = await dbcon()('OPT_KV')
            .where({
                key: 'CURRENT_VERSION',
            })
            .select('value');
        ...
        if (
            stepVersionResult.length === 1 &&
            stepVersionResult[0].value !== undefined
        )
    ...
    if (stepVersion !== CURRENT_VERSION) {
        if (CURRENT_VERSION < stepVersion)
            return Promise.reject(
                new Error(
                    'The existing database was created with a newer version of Optimise ! Please upgrade before using Optimise !'
                )
            );
