...
export async function migrate() {
    ...
    const isIntitialized = await db.collections().then(cols => cols.filter(col => col.collectionName === 'OPT_ASSIST_KV')).then(res => res.length > 0);
    let stepVersion = 0;

    if (!isIntitialized) {
        // If not create it and initialize CURRENT_VERSION
        await db.createCollection('OPT_ASSIST_KV');
        await db.collection('OPT_ASSIST_KV').insertOne({
            key: 'CURRENT_VERSION',
            value: '0',
        ...
        const stepVersionResult = await db.collection('OPT_ASSIST_KV').findOne({
            key: 'CURRENT_VERSION'
        });
    ...
    if (stepVersion !== CURRENT_VERSION) {
        ...
        await db.collection('OPT_ASSIST_KV').updateOne({ key: 'CURRENT_VERSION' }, {
            $set: {
                value: `${stepVersion}`,
                updated_at: (new Date()).getTime()
            }
