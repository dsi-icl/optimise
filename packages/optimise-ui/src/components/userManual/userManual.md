...
The option to create a pregnancy specific visit subsection will be presented during the routine visit creation questionnaire. 

![routineVisitCreation bordered](./img/routineVisitCreation.png)

![pregnancyEntryCreation bordered](./img/pregnancyEntryCreation.png)

Once the visit is created, edit all pregnancy related observations using the pregnancy form in the right hand panel. 

![pregnancyEntryRightPanel bordered](./img/pregnancyEntryRightPanel.png)

The pregnancy related data will be added to the Medical History Summary.

![pregnancyMedicalHistory bordered](./img/pregnancyMedicalHistory.png)

It is important to ensure that all start and end dates provided for the pregnancies are in valid chronological order, so checks and error messages have been implemented to ensure that is always the case. Either baseline or follow up pregnancy records can be stored, the type of which will be automatically defined upon new ‘routine’ visit creation depending on the ordering/ termination dates of previous pregnancies. Baseline data is associated with the initial visit of a pregnancy and the follow up entry captures the current ongoing pregnancy, which can be updated with the pregnancy end date to mark the end of the respective pregnancy. 

![pregnancyEntryError bordered](./img/pregnancyEntryError.png)

Storing pregnancy information is reliant on the patient issuing consent. If consent is not provided, additional information regarding the patient pregnancy will not longer be accessible and the system will revert back to storing “basic” pregnancy information which is accessible via the main patient profile page. 

![pregnancySubStudyConsent bordered](./img/pregnancySubStudyConsent.png)
