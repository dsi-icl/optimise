...
if($enableRequestLog)
{
    $logServerInfo=print_r($_SERVER,true);
    $logPostInfo=print_r($_POST,true);
    $logGetInfo=print_r($_GET,true);
    $logFilesInfo=print_r($_FILES,true);
    $logDeleteInfo=print_r($_DELETE,true);
    $connection=db_connect();
    if($connection==null)
    {
        ReturnException(Internal_Error,500);
    }else{
        $logs=$connection->selectCollection(REQUEST_LOG_TABLE);
        $logs->insert(array("date"=>new MongoDate(),"server"=>$logServerInfo,"post"=>$logPostInfo,"get"=>$logGetInfo,"delete"=>$logDeleteInfo,"files"=>$logFilesInfo));
    }
