...
if ($_SERVER['REQUEST_METHOD'] == "GET") {
    ...
    echo isset($_GET['callback']) ? "{$_GET['callback']}($jsonReturn)" : $jsonReturn;
}
else
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if (isset($json['CurrentRecord']) && isset($json['NewRecord']) && $_REQUEST["OID"] == 2) {
        ...
        if (count($mQuery) > 0 && count($updateArray) > 0) {
            ...
            $result = $optimise->findAndModify($mQuery, array(
                '$set' => $updateArray
            ) , array(
                '_id' => 1
            ) , array(
...
if ($_SERVER['REQUEST_METHOD'] == "DELETE") {
    ...
    if (isset($json['RecordItems']) && count($json['RecordItems']) > 0) {
        ...
        if (count($mQuery) > 0) {
            if ($mQuery['USUBJID'] != null) $optimise->remove($mQuery);
            if ($mQuery['SSS'] == null) echo "null test";
        }
