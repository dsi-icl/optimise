...
laboratoryTestResultModule.controller('laboratoryInfoCtrl', function($scope, $rootScope, $filter, NgTableParams,
    ...
    $rootScope.displayResults = function(DOMAIN) {
        ...
        else
        {
            ...
            if (resultsOfDTC.length > 0)
            {
                ...
                //$scope.LBDTC_displayDate = $scope.LBDTC.getDate()+"/"+(parseInt($scope.LBDTC.getMonth()+1))+"/"+$scope.LBDTC.getFullYear();// set date
                for (r = 0; r < resultsOfDTC.length; r++){
                    scopeVariables = haematologyVocab.getLabTestScopeName(resultsOfDTC[r].LBTEST);
            ...
            if (assessmentOfDTC.length > 0)
            {
                ...
                for (r = 0; r < assessmentOfDTC.length; r++){
                    scopeVariables = serologicalTestsVocab.getAssessmentScopeName(assessmentOfDTC[r].ISTEST);
            ...
            if (nsFindingsOfDTC.length > 0)
            {
                ...
                for (r = 0; r < nsFindingsOfDTC.length; r++){
                    var modelTestValue = null;
                    if (nsFindingsOfDTC[r].NVLOC == ''){
                        //console.log(nsFindingsOfDTC[r]);
                        scopeVariables = getVEPScopeName(nsFindingsOfDTC[r].NVTEST, nsFindingsOfDTC[r].NVLAT);
                        if (scopeVariables != null) {
                            modelTestValue = $parse(scopeVariables.scopeVariable);
    ...
    var clearFields = function() {
        ...
        for (k = 0; k < $scope.haematologyScopeVariables.length; k++){
        ...
        for (k = 0; k < $scope.assessmentScopeVariables.length; k++){
        ...
        for (k = 0; k < $scope.bloodChemScopeVariables.length; k++){
        ...
        for (k = 0; k < $scope.thyroidScopeVariables.length; k++){
